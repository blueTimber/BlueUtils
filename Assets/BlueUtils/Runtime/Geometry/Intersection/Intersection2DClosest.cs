...
namespace BlueUtils.Geometry
{
	public static partial class Intersection2D
	{
		...
		public static (Vector2, Vector2)? ClosestPointBetweenLineSegments(Vector2 line1start, Vector2 line1end, Vector2 line2start, Vector2 line2end)
		{
			...
			// Line 2 start
			(Vector2, Vector2) candidate = (ClosestPointOnLineSegment(line1start, line1end, line2start), line2start);
			...
			// Line 2 end
			candidate = (ClosestPointOnLineSegment(line1start, line1end, line2end), line2end);
			if ((candidate.Item2 - candidate.Item1).sqrMagnitude < minSqrDistance)
			{
				minSqrDistance = (candidate.Item2 - candidate.Item1).sqrMagnitude;
				closestPoints = candidate;
			...
			// Line 1 start
			candidate = (ClosestPointOnLineSegment(line2start, line2end, line1start), line1start);
			if ((candidate.Item2 - candidate.Item1).sqrMagnitude < minSqrDistance)
			{
				minSqrDistance = (candidate.Item2 - candidate.Item1).sqrMagnitude;
				closestPoints = candidate;
			...
			// Line 1 end
			candidate = (ClosestPointOnLineSegment(line2start, line2end, line1end), line1end);
			if ((candidate.Item2 - candidate.Item1).sqrMagnitude < minSqrDistance)
			{
				// minSqrDistance = (candidate.Item2 - candidate.Item1).sqrMagnitude;
				closestPoints = candidate;
